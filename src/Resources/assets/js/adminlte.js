    ...
    class Treeview {
        ...
        toggle() {
            ...
            if ((_a = this._navItem) === null || _a === void 0 ? void 0 : _a.classList.contains(CLASS_NAME_MENU_OPEN)) {
                console.error(this, this._navItem, _a, _a.classList);
                this.close();
            }
            else {
                this.open();
    ...
    domReady(() => {
        ...
        for (const btn of button) {
            btn.addEventListener('click', event => {
                ...
                // const _a = currentTarget.node.first('a');
                // console.error(_a);
                console.error('calling toggle.', this);
                // console.warn(treeviewMenu, event.currentTarget, event, btn, currentTarget.classList);
                const data = new Treeview(treeviewMenu, Default$1);
                data.toggle();
                //
                // // console.warn(event.elecurrentTarget.nodeName, currentTarget.classList);
                // if (currentTarget.classList.contains('leaf')) {
                //     console.error('not toggling', currentTarget);
                //     // could prevent default and load the page here.
                // } else {
                // }
            });
        }
